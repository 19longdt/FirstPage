...
//        if(!img.equals(p.getImg()))
        

        int n = dao.updateProduct(new Product(Integer.parseInt(id), name, Double.parseDouble(price),
                Integer.parseInt(amount), img, Integer.parseInt(cid), des));
        if (n == 1) {
            request.setAttribute("result", "Update Successfull");
            Product p1 = dao.getProductById(id);
